...
class ProposalReviewController extends Controller
{
    ...
    public function viewProposals($id)
    {
    	...
    	return view('proposals.reviews.index',['title'=>'Received Proposals','proposals'=>$proposals]);
    }
...

    {
    	...
    	{
    		return Redirect::back()->withErrors('This freelancer is already shortlisted for this job');
    	}
    	...
    	//Fire and event
                ...
                return Redirect::back()->with('message','Added to shortlist');
...

    {
        /*$shortlists = DB::table('short_lists')
    					->join('jobs','job_id','short_lists.job_id','jobs.id')
    					->join('proposals','proposal_id','short_lists.proposal_id','proposals.id')
    					->join('freelancers','short_lists.freelancer_id','freelancers.id','short_lists.freelancer_id')
...
return view('proposals.reviews.shortlist',['title'=>'Short listed Candidates','shortlists'=>$shortlists]);
    }
