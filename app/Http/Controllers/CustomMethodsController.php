...
class CustomMethodsController extends Controller
{
    ...
    {
    	$jobs =Job::where('job_status',1)->where('date_line','>',Carbon::now())->paginate(15);

    	  return view('jobs.index',['title'=>'All active Jobs','jobs'=>$jobs]);


    }


    //Create freelancer Award

    public function createAward()
    {
    	return view('portfolios.awards.create',['title'=>'Add New Award']);
    }
...
dd($position);*/
  ...
  $skill = Skill::where('skill',$term)->first();
    ...
    $title = 'Jobs that match search term: '.$term;

  return view('jobs.index',['title'=>$title,'jobs'=>$jobs]);
